...
-- MAGIC %md
-- MAGIC ## order_status_event — Validation Narrative
-- MAGIC 
-- MAGIC **Entity:** `order_status_event` · **Tier:** 5 · **Type:** FACT · **Strategy:** MV (full recompute)
-- MAGIC 
-- MAGIC **Surrogate Key:** `order_status_event_id`  
...
DELETE FROM _validation_check_detail WHERE Run_Id='PENDING' AND Table_Name='order_status_event';
INSERT INTO _validation_check_detail
-- PK
SELECT 'PENDING','order_status_event','PK_Uniqueness','PK',
  CASE WHEN COUNT(*)=COUNT(DISTINCT order_status_event_id) THEN 'PASS' ELSE 'FAIL' END,
  '0',CAST(COUNT(*)-COUNT(DISTINCT order_status_event_id) AS STRING),NULL,FALSE,CAST(NULL AS STRING),
  'PK: '||CASE WHEN COUNT(*)=COUNT(DISTINCT order_status_event_id) THEN 'PASS' ELSE 'FAIL' END,current_timestamp()
FROM order_status_event
UNION ALL
-- FK order_id
SELECT 'PENDING','order_status_event','FK_order_id','FK',
...
FROM order_status_event t LEFT JOIN `order` p ON t.order_id=p.order_id
UNION ALL
-- BK
SELECT 'PENDING','order_status_event','BK_Null_Check','BK',
...
UNION ALL
-- POP
SELECT 'PENDING','order_status_event','POP_Key_Columns','POP',
...
FROM (SELECT COUNT(source_system_line_key)*100.0/COUNT(*) AS p FROM order_status_event
  UNION ALL SELECT COUNT(event_type)*100.0/COUNT(*) AS p FROM order_status_event
  UNION ALL SELECT COUNT(event_timestamp)*100.0/COUNT(*) AS p FROM order_status_event)
UNION ALL
-- INTEGRATION
SELECT 'PENDING','order_status_event','INTEG_Join_Parent','INTEGRATION',
  CASE WHEN j=b THEN 'PASS' ELSE 'FAIL' END,
  CAST(b AS STRING),CAST(b AS STRING),
  NULL,FALSE,CAST(NULL AS STRING),
  'INTEG: base='||CAST(b AS STRING),current_timestamp()
FROM (SELECT (SELECT COUNT(*) FROM order_status_event) AS b, (SELECT COUNT(*) FROM order_status_event e LEFT JOIN `order` o ON e.order_id=o.order_id) AS j)
UNION ALL
-- DRIFT
SELECT 'PENDING','order_status_event','DRIFT_Row_Count','DRIFT',
...
FROM (SELECT COUNT(*) AS cnt FROM order_status_event)
LEFT JOIN _data_drift_baseline b ON b.Table_Name='order_status_event' AND b.Column_Name='order_status_event_id' AND b.Metric_Type='ROW_COUNT';
